/**
 * Sentence Build Exercise Type
 * Build sentences from word prompts
 */
...
const SentenceBuildExercise = {
  render(question, container, callbacks) {
    ...
    }, question.instructions || 'Write a sentence using the words provided');

    const promptBox = Utils.createElement('div', {
      class: 'exercise__question'
    });
    promptBox.innerHTML = `<strong>Prompt:</strong> ${question.prompt}`;
    ...
    submitBtn.onclick = () => {
      ...
      if (Utils.isEmpty(userAnswer)) {
        Utils.showToast('Please write a sentence', 'error');
        return;
      }
    ...
    exerciseCard.appendChild(promptBox);
...
if (typeof GameEngine !== 'undefined') {
  GameEngine.registerExerciseHandler('sentence-build', SentenceBuildExercise);
}
